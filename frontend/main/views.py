...
from project.models import GeneratedTestCase, ProjectDocument, UserProject
...
def home(request):
    ...
    all_recent_documents = ProjectDocument.objects.filter(
        project__in=user_projects
    ...
    avg_daily_7_days = (
        round(last_7_days_activities / 7, 1) if last_7_days_activities > 0 else 0
    )
    max_daily_7_days = (
        max(item["total"] for item in request_frequency_data[-7:])
        if request_frequency_data[-7:]
        else 0
    )
...
def library(request):
    ...
    all_documents = ProjectDocument.objects.filter(project__user=request.user).order_by(
        "-uploaded_at"
    )
...
def reports(request):
    ...
    all_reports = TestSuiteReport.objects.filter(project__user=request.user).order_by(
    ...
    if project_uuid:
        try:
            selected_project = UserProject.objects.get(
            ...
            messages.warning(
                request, "Dự án không tồn tại hoặc bạn không có quyền truy cập."
            )
    ...
    user_projects = UserProject.objects.filter(user=request.user).order_by(
        "-created_at"
    )
...
def set_language(request):
    ...
    if request.method == "POST":
        ...
        if language in [lang[0] for lang in settings.LANGUAGES]:
            ...
            response.set_cookie(
                settings.LANGUAGE_COOKIE_NAME, language, max_age=365 * 24 * 60 * 60
            )  # 1 year
            return response
